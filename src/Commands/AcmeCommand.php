...
class AcmeCommand extends TerminusCommand implements SiteAwareInterface
{
     ...
     */
    public function writeAcmeFile($site_env, $domain)
    {
     ...
     */
    public function getAcmeDNStxt($site_env, $domain, $options = ['format' => 'list'])
    {
      list($data, $acmeStatus) = $this->getACMEStatus($site_env, $domain);
      if (!$acmeStatus) {
        return;
      ...
      $data = $data->{'dns-01'};
      $struct = AcmeCommand::formatAcmeDNStxt($domain, $data);
      $txt_record = $struct[0];
    ...
    public static function formatAcmeDNStxt($domain, $data) {
