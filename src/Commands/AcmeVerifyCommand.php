...
/**
 * Class AcmeVerifyCommand
 * @package Pantheon\Terminus\Commands\HTTPS
 */
class AcmeVerifyCommand extends TerminusCommand implements SiteAwareInterface {
  use SiteAwareTrait;

  /**
   * Triggers acceptance of the ACME http-01 challenge returned by https:acme:file
   * and prints whether ownership of the domain was successfully proven.
   *
   * @authorize
   *
   * @command https:acme:file:verify
   ...
   */
  public function verifyHttpAcme($site_env, $domain)
  {
    $this->verifyAcme($site_env, $domain, 'http-01');
  }

  /**
   * Triggers acceptance of the ACME dns-01 challenge returned by https:acme:dns
   * and prints whether ownership of the domain was successfully proven.
   *
   * @authorize
   *
   * @command https:acme:dns-txt:verify
   ...
   */
  public function verifyDnsAcme($site_env, $domain)
  {
    $this->verifyAcme($site_env, $domain, 'dns-01');
  }

  protected function verifyAcme($site_env, $domain, $challenge_type) {
    ...
    if (!empty($data->acme_preauthorization_challenges)) {
      ...
      if ($new_challenge != $current_challenge) {
        ...
        if ($challenge_type == 'dns-01') {
          $struct = AcmeCommand::formatChallengeDNStxt($domain, $data->acme_preauthorization_challenges->{'dns-01'});
          $txt_record = $struct[0];
